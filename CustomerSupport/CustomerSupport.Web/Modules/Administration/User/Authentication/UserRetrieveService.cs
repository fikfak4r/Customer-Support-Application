namespace CustomerSupport.Administration
{
    ...
    public class UserRetrieveService : IUserRetrieveService
    {
        ...
        public IUserDefinition ByEmail(string email)
        {
            if (email.IsEmptyOrNull())
                return null;

            return TwoLevelCache.Get<UserDefinition>("Email_" + email.ToLowerInvariant(),
                TimeSpan.Zero, TimeSpan.FromDays(1), fld.GenerationKey, () =>
                {
                    using (var connection = SqlConnections.NewByKey("Default"))
                        return GetFirst(connection, new Criteria(fld.Email) == email);
                });
        }
