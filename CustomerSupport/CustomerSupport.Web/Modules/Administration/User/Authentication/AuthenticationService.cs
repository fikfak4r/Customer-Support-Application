namespace CustomerSupport.Administration
{
    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        private string ResolveUserName(string userNameOrEmail)
        {
            ...
            using (var conn = SqlConnections.NewFor<UserRow>())
            {
                if (conn.Exists<UserRow>(new Criteria("Email") == userNameOrEmail))
                {
                    UserRow userRow = conn.Single<UserRow>(new Criteria("Email") == userNameOrEmail);
                    if (userRow != null)
                        userNameOrEmail = userRow.Username;
                }
        ...
        public bool Validate(ref string username, string password)
        {
            ...
            username = ResolveUserName(username.TrimToEmpty());

            var user = Dependency.Resolve<IUserRetrieveService>().ByUsername(username) as UserDefinition;

            if (user == null && username.Contains("@"))
                user = Dependency.Resolve<UserRetrieveService>().ByEmail(username) as UserDefinition;
        ...
        public static bool DoesBusinessExist(IDbConnection conn, String businessName)
        {
            bool rtnVal=false;
            IDataReader reader = new SqlQuery()
             .From("Accounts")
            .Select("CompanyName")
            .Where(new Criteria("CompanyName") == businessName)
            .ExecuteReader(conn);
        ...
        public static bool DoesEmailExist(IDbConnection conn, String email)
        {
            return conn.Exists<UserRow>(new Criteria("Email") == email);
        }
