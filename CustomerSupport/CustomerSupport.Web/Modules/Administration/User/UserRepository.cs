    ...
    public partial class UserRepository
    {
        ...
        private class MySaveHandler : SaveRequestHandler<MyRow>
        {
            ...
            protected override void GetEditableFields(HashSet<Field> editable)
            {
                ...
                if (!Authorization.HasPermission(Administration.PermissionKeys.Security))
                {
                    ...
                    throw new Exception("Got in here");
                }

                //throw new Exception("Right in here 2");
        ...
        private class MyListHandler : ListRequestHandler<MyRow> 
        {
            ...
            protected override void ApplyFilters(SqlQuery query)
            {
                ...
                //var x = Entities.UserLocationRow.Fields.As("x");
                //Gets users under his/watch whlie excluding his own and customers records
                query.Where(
                    String.Format(@"UserId IN 
                    (SELECT UserId FROM UsersLocations WHERE LocationId IN(SELECT LocationId FROM UsersLocations WHERE UserId = {0})) AND UserId <> {0} AND CustomerID = 0", user.UserId));
            }
        }
